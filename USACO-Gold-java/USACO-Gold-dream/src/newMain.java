...
class newMain {  
  public static void main(String[] args) throws IOException {
    ...
    while (queue.size() > 0) {
      ...
      int numSteps = node[2];
      int isOrange = node[3];
      if (visited[location[0]][location[1]][isOrange]) {
        continue;
      }
      visited[location[0]][location[1]][isOrange] = true;
      ...
      for (int i = 0; i < 4; i++) {
        ...
        if (!visited[newLocation[0]][newLocation[1]][isOrange]) {
            if (val == 0) {
                //no pass
            } else if (val == 1) {
                queue.add(new int[]{newLocation[0], newLocation[1], numSteps + 1, isOrange});
            } else if (val == 2) {
                ...
                while (true) {
                  steps += 1;
                  newLocation[0] += directionArray[i][0];
                  newLocation[1] += directionArray[i][1];
